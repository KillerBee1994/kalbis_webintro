...
{
	//ambil nilai kelas
	kelas = document.getElementById("class").value;
	...
	posisi_kursi = kursi_eco.length;
		...
		if(sisa_kursi_eco>= 1 && jml <= sisa_kursi_eco)
		{
			...
			for(d=1;d<=jml;d++)
			{
				...
				document.getElementById("notif").innerHTML = "Kursi Ekonomi telah dipesan, Terima Kasih";
				kursi_eco[index] = "x";
			}
		}
		else
	...
	posisi_kursi= kursi_bus.length;
		...
		if(sisa_kursi_bus>= 1 && jml <= sisa_kursi_bus)
		{
			...
			for(d=1;d<=jml;d++)
			{
				...
				index = next_kursi - 1; 
				document.getElementById(next_kursi).innerHTML = "x";
				document.getElementById("notif").innerHTML = "Kursi Bisnis telah dipesan, Terima Kasih";
				kursi_bus[index] = "x";
			}
		}
		else
